...
@code{
    ...
    private void Run()
    {
        ComponentType = CodeRenderingHelper.RenderingToType(new RenderingToTypeOptions()
        {
...
### 添加编译时的全局引用
```csharp
CodeRenderingProject.AddGlobalUsing("@using Masa.Blazor")
```

### 删除编译时的全局引用
```csharp
CodeRenderingProject.RemoveGlobalUsing("@using Masa.Blazor")
```
